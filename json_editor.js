;(function($){
	...
	const self={
		...
		header:function(){
			...
			const moreID=hash(),keyID=hash(),enableID=hash();
			const addKey=hash(),addValue=hash(),btnAdd=hash();
		...
		loop:function(obj,dep,skip,disable,chain){
			if(!dep) dep=0;
			let str='';
			const isShow=self.isShow,domRow=self.domRow,getKey=self.getChainKey,getID=self.getIDbyChain;
			const left=config.row.left;right=config.row.right,ncol=config.row.note;
			const clsTitle=config.clsTitle,clsBody=config.clsBody,clsNote=config.clsNote,col=config.clsCol,clsRight=config.clsRight;
			const clsHide=config.clsHide,clsRow=config.clsRow;
			for(const k in obj){
				if(dep==0) chain=[];
		...
		operationButtons:function(k,chain){
			const idRemove=hash(),idHide=hash(),idShow=hash();
			const hkey=self.getChainKey(chain,k);
			agent.push(function(){
		...
		getEnum:function(id,v,disable,list){
			...
			for(let i=0,len=list.length;i<len;i++){
				const row=list[i],set=row.value==v?'selected="selected"':'';
		...
		save:function(val,chain,force){
			let tmp=cache;
			const recover=self.recoverSpace,cnt=config.connector;
